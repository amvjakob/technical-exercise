...
const CalendarBox: React.FC = () => {
  ...
  const dataCalendar = data?.map((item: any) => ({
    id: item.id,
    title: item['Experiment Type'],
    start: item['Start Date_raw'].toISOString().replace(/T.*$/, ''),
    end: item['End Date_raw'].toISOString().replace(/T.*$/, ''),
  }));
  ...
  console.log('initalEvents', INITIAL_EVENTS);
