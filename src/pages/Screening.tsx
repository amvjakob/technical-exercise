...
import { getDataScreening } from '../api/ApiCollection';
import { useQuery } from '@tanstack/react-query';
import toast from 'react-hot-toast';

const Screening = () => {
  const { isLoading, isError, isSuccess, data } = useQuery({
    queryKey: ['all_screening'],
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'well', headerName: 'Plate well ID', minWidth: 100, flex: 1 },
    { field: 'compound', headerName: 'Internal compound ID', minWidth: 100, flex: 1 },
    { 
      field: 'hit',
      headerName: 'Hit',
      minWidth: 100,
          ...
          <div className="flex gap-1 justify-start flex-col items-start">
            ...
            {data && data.length > 0 && (
              <span className="text-neutral dark:text-neutral-content font-medium text-base">
                {data.length} screening experiments found
              </span>
            )}
          </div>
        </div>
